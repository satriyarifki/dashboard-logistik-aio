...
@Component({
  selector: 'app-input-budget-factory',
  templateUrl: './input-budget-factory.component.html',
  styleUrls: ['./input-budget-factory.component.css'],
})
export class InputBudgetFactoryComponent {
  //TOOLS
  ...
  ngOnInit() {
    ...
    forkJoin(
      ...
      this.apiService.getBudgetShipping(),
      this.apiService.getBudgetFactory()
    ).subscribe(
      (res) => {
        this.rmpmViewApi = res[0];
        this.rmpmViewGroupApi = res[1];
        this.rmpmStorageApi = res[2];
        this.budgetShippingApi = res[3];
        this.budgetFactoryApi = res[4];
        // console.log(res[2]);
